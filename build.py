...
LB_HTML = """<div class="lb" id="lb" onclick="if(event.target===this)dongLb()">
...
function moLb(ds, i){
  ...
  document.getElementById('lb-anh').src = lbDs[lbI];
  document.getElementById('lb-dem').textContent =
    lbDs.length > 1 ? (lbI+1) + ' / ' + lbDs.length : '';
  document.querySelectorAll('.lb .truoc,.lb .sau').forEach(function(b){
...
function luot(d){
  ...
  document.getElementById('lb-anh').src = lbDs[lbI];
  document.getElementById('lb-dem').textContent = (lbI+1) + ' / ' + lbDs.length;
}
