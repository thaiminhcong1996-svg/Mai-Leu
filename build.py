...
def thu_vien(anh, ten, trong="chưa có ảnh"):
    ...
    ds = E(json.dumps(anh, ensure_ascii=False))
    if len(anh) == 1:
        return ('<div class="gal" data-anh="%s"><div class="gal-main">'
                '<img src="%s" alt="%s"></div></div>' % (ds, E(anh[0]), E(ten)))
    ...
    return ('<div class="gal" data-anh="%s"><div class="gal-th">%s</div>'
            '<div class="gal-main"><img id="anh-lon" src="%s" alt="%s"></div></div>'
            % (ds, th, E(anh[0]), E(ten)))

LB_HTML = """<div class="pto" id="pto" style="display:none" onclick="if(event.target===this)dongLb()">
  <button class="dong" onclick="dongLb()" aria-label="Đóng">✕</button>
  <button class="truoc" onclick="event.stopPropagation();luot(-1)" aria-label="Ảnh trước">‹</button>
  <img id="pto-anh" src="" alt="">
  <button class="sau" onclick="event.stopPropagation();luot(1)" aria-label="Ảnh sau">›</button>
  <span class="dem" id="pto-dem"></span>
</div>"""

GAL_JS = LB_HTML + """<script>
var lbDs=[], lbI=0;
function moLb(ds, i){
  lbDs = ds || []; lbI = i || 0;
  if(!lbDs.length) return;
  document.getElementById('pto-anh').src = lbDs[lbI];
  document.getElementById('pto-dem').textContent =
    lbDs.length > 1 ? (lbI+1) + ' / ' + lbDs.length : '';
  document.querySelectorAll('.pto .truoc,.pto .sau').forEach(function(b){
    b.style.display = lbDs.length > 1 ? '' : 'none'; });
  var lb=document.getElementById('pto');
  lb.style.display='flex'; lb.classList.add('on');
  document.body.style.overflow = 'hidden';
}
function dongLb(){
  var lb=document.getElementById('pto');
  lb.style.display='none'; lb.classList.remove('on');
  document.body.style.overflow = '';
}
function luot(d){
  if(!lbDs.length) return;
  lbI = (lbI + d + lbDs.length) % lbDs.length;
  document.getElementById('pto-anh').src = lbDs[lbI];
  document.getElementById('pto-dem').textContent = (lbI+1) + ' / ' + lbDs.length;
}
document.addEventListener('keydown', function(e){
  var lb=document.getElementById('pto');
  if(!lb || lb.style.display==='none') return;
  if(e.key === 'Escape') dongLb();
  if(e.key === 'ArrowLeft') luot(-1);
  if(e.key === 'ArrowRight') luot(1);
});
/* bấm vào ảnh lớn hoặc ảnh trong dải ngang */
document.addEventListener('click', function(e){
  var img = e.target.closest('.gal-main img, .shots img');
  if(!img) return;
  var hop = img.closest('.gal, .shots');
  var ds = [];
  try { ds = JSON.parse(hop.dataset.anh || '[]'); } catch(x){}
  if(!ds.length) ds = [img.src];
  var i = ds.indexOf(img.getAttribute('src'));
  moLb(ds, i < 0 ? 0 : i);
});
</script>
<script>
